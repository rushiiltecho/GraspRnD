...
class ObjectDetector:
    ...
    def detect_objects(self, image_path: Optional[str]= None, image: Optional[Image.Image] = None, target_class: str= None) -> List[Dict]:
        ...
        try:
            ...
            prompt_text = (
            "Return bounding boxes for object in the"
            f" following format as a list. \n ```json{'{ '}{target_class if target_class else '''<name of object in the frame>'''} : [xmin, ymin, xmax, ymax]"
            " ...}``` \n If there are more than one instance of an object, add"
    ...
    def __deprecated_get_object_centers(self, im: Image, target_classes: List[str]) -> Dict[str, Tuple[Optional[int], Optional[int], Optional[np.ndarray], Optional[float]]]:
        """
