...
from audio.system_prompts.alice_prompt import alice_prompt
...
class OpenAIInteractiveVoiceClient:
    """
    ...
    def __init__(
        ...
        on_input_transcript: Optional[Callable[[str], None]] = lambda transcript: print(f"\nUser: {transcript}", end="", flush=True),  
        on_output_transcript: Optional[Callable[[str], None]] = lambda transcript: print(f"\nAI: {transcript}", end="", flush=True),  
        extra_event_handlers: Optional[Dict[str, Callable[[Dict[str, Any]], None]]] = None,
        message_ai_manager  = None,
        message_user_manager = None,
        audio_manager = None
    ):
        ...
        self.message_ai_manager= message_ai_manager
        self.message_user_manager = message_user_manager
    ...
    async def call_tool(self, call_id: str,tool_name: str, tool_arguments: Dict[str, Any]) -> None:
        """Execute a tool call and send the result back to the API."""
            ...
            async for message in self.ws:
                ...
                # Handle normal response events
                    ...
                    if self.on_audio_delta:
                        ...
                        # Create a structured message for audio streaming
                        # audio_message = {
                        #     "type": "audio_delta",
                        #     "audio": audio_bytes
                        # }
                        # Send the audio message to the frontend
                        # audio_send= base64.b64encode(audio_bytes).decode('utf-8')
                        # await self.audio_manager.broadcast_data(audio_send)
                ...
                # Handle input audio transcription
                    ...
                    if self.on_input_transcript:
                        # await self.message_user_manager.send_message(json.dumps({"response_from":"user","text":transcript}))
                ...
                # Handle output audio transcription
                    ...
                    if self.on_output_transcript:
                        ...
                        if not self._print_input_transcript:
                            ...
                            message_transcript=json.dumps({"response_from":"AI","text":self._output_transcript_buffer})
                            # await self.message_manager.broadcast_data(json.dumps({"response_from":"AI","text":self._output_transcript_buffer}))
                        else:
                            if self._output_transcript_buffer:
                                self._output_transcript_buffer += delta  # Append new text to buffer
                            print(self._output_transcript_buffer)
                            await asyncio.to_thread(self.on_output_transcript, self._output_transcript_buffer)  # Send entire buffer
                            message_transcript= json.dumps({"response_from":"AI","text":self._output_transcript_buffer})
                        print(message_transcript)
                        # await self.message_ai_manager.send_message(message_transcript)
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            if self.ws:
                # Get the event loop explicitly
                loop = asyncio.get_running_loop()
                
                # Ensure we're closing on the correct event loop
                if self.ws._loop != loop:
                    raise RuntimeError("WebSocket attached to different event loop")
            ...
            # Attempt force close if normal close fails
            if self.ws:
                try:
                    await self.ws.close(code=1000, reason="Force close during cleanup")
                except:
                    pass
                self.ws = None