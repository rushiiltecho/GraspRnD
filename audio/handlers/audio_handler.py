...
import threading
import time
from audio.voice_clients import OpenAIInteractiveVoiceClient


class AudioHandler:
    """
    Handles audio input and output for the Alice Agent.
    ...
    """
    def __init__(self, rate=24000, chunk=1024):
        # Audio parameters
        self.mic_muted = False
        self.previous_streaming_state = False
    ...
    async def start_streaming(self, client: OpenAIInteractiveVoiceClient):
        ...
        while self.streaming:
            try:
                if not self.mic_muted:  # Only stream audio when mic is not muted
                    # Read raw PCM data
                    data = self.stream.read(self.chunk, exception_on_overflow=False)
                    # Stream directly without trying to decode
                    await client.stream_audio(data)
                else:
                    # If muted, we still need to read from the stream to prevent buffer overflow
    ...
    def play_audio(self, audio_data: bytes):
        """Add audio data to the buffer and mute microphone while playing"""
        # Mute the microphone before starting playback
    ...
    def _continuous_playback(self):
        ...
        while not self.stop_playback:
            try:
                ...
                # If the buffer is empty, we can unmute the microphone
        ...
        # Make sure to unmute the microphone when playback is done
        self.unmute_microphone()
